...
return function(Iris: Types.Internal, widgets: Types.WidgetUtility)
	...
	Iris.WidgetConstructor("InputInstance", {
		hasState = true,
		hasChildren = false,
		Args = {
			["Text"] = 1,
		...
		Events = {
			["instanceChanged"] = {
				["Init"] = function(thisWidget: Types.InputInstance)
					thisWidget.lastInstanceChangedTick = 0
				end,
				["Get"] = function(thisWidget: Types.InputInstance)
					return thisWidget.lastInstanceChangedTick == Iris._cycleTick
				end,
			},
			["hovered"] = widgets.EVENTS.hover(function(thisWidget: Types.Widget)
				return thisWidget.Instance
		...
		Generate = function(thisWidget: Types.InputInstance)
			local InputInstance: Frame = Instance.new("Frame")
			InputInstance.Name = "Iris_InputInstance"
			InputInstance.AutomaticSize = Enum.AutomaticSize.Y
			InputInstance.Size = UDim2.fromScale(1, 0)
			InputInstance.BackgroundTransparency = 1
			InputInstance.BorderSizePixel = 0
			InputInstance.ZIndex = thisWidget.ZIndex
			InputInstance.LayoutOrder = thisWidget.ZIndex
			local UIListLayout: UIListLayout = widgets.UIListLayout(InputInstance, Enum.FillDirection.Horizontal, UDim.new(0, Iris._config.ItemInnerSpacing.X))
			UIListLayout.VerticalAlignment = Enum.VerticalAlignment.Center
			...
			widgets.applyFrameStyle(InputField)
			widgets.applyTextStyle(InputField)
			InputField.TextColor3 = Iris._config.TextDisabledColor
			widgets.UISizeConstraint(InputField, Vector2.xAxis) -- prevents sizes beaking when getting too small.
			-- InputField.UIPadding.PaddingLeft = UDim.new(0, Iris._config.ItemInnerSpacing.X)
			-- InputField.UIPadding.PaddingRight = UDim.new(0, 0)
			InputField.Parent = InputInstance

			InputField.MouseButton1Click:Connect(function()
				InputField.TextColor3 = Iris._config.TextDisabledColor
				InputField.Text = thisWidget.arguments.SelectionHint or "Waiting for Selection ..."
				
				Selection.SelectionChanged:Wait()
				
				local selection = Selection:Get()
				
				local inst = selection[#selection]
				
				if inst and (not thisWidget.arguments.IsA or inst:IsA(thisWidget.arguments.IsA)) then
					thisWidget.state.instance:set(inst)
					thisWidget.lastInstanceChangedTick = Iris._cycleTick + 1
					InputField.TextColor3 = Iris._config.TextColor
					InputField.Text = inst.Name
				else
					thisWidget.state.instance:set(nil)
					InputField.TextColor3 = Iris._config.TextDisabledColor
					InputField.Text = thisWidget.arguments.ErrorText or "Invalid Selection"
				end
			end)
			...
			TextLabel.Parent = InputInstance

			return InputInstance
		end,
		Update = function(thisWidget: Types.InputInstance)
			local InputInstance = thisWidget.Instance :: Frame
			local TextLabel: TextLabel = InputInstance.TextLabel
			local InputField: TextButton = InputInstance.InputField

			TextLabel.Text = thisWidget.arguments.Text or "Input Instance"
			InputField.Text = thisWidget.arguments.TextHint or "Click to Select"
		end,
		Discard = function(thisWidget: Types.InputInstance)
			thisWidget.Instance:Destroy()
			widgets.discardState(thisWidget)
		end,
		GenerateState = function(thisWidget: Types.InputInstance)
			if thisWidget.state.instance == nil then
				thisWidget.state.instance = Iris._widgetState(thisWidget, "instance", nil)
			end
		end,
		UpdateState = function(thisWidget: Types.InputInstance)
			local InputText = thisWidget.Instance :: Frame
			local InputField: TextBox = InputText.InputField
			
			local value = thisWidget.state.instance.value

			if value then
				InputField.Text = value.Name
			end
		end,
	} :: Types.WidgetClass)
end
